...
export default function TopJudgesOutcomeChart({ data }: { data?: Row[] }) {
      ...
      <ResponsiveContainer width="100%" height={300}>
        {rows.length === 0 ? (
          <div className="h-full w-full flex items-center justify-center text-sm text-[#5f6368]">
            No judge-outcome data for this lawyer.
          </div>
        ) : (
          <BarChart data={rows} layout="vertical" margin={{ top: 8, right: 12, left: -12, bottom: 0 }}>
            ...
            <Bar dataKey="settled" name="Settled" stackId="a" fill="#92400e" barSize={24} />
          </BarChart>
        )}
      </ResponsiveContainer>
    </div>
