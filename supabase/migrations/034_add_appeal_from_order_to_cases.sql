-- Add "Appeal from Order" to master cases and wire it into admin JSON import.

BEGIN;

ALTER TABLE public.cases
  ADD COLUMN IF NOT EXISTS appeal_from_order text NULL;

COMMENT ON COLUMN public.cases.appeal_from_order IS 'Appeal from Order (free text: yes/no, reference, or description).';
...
BEGIN
  ...
  FROM (
    SELECT DISTINCT ON (r.case_number)
      ...
      NULLIF(btrim(r.appeal_from_order), '') AS appeal_from_order,
  ...
  ON CONFLICT (case_number) DO UPDATE
    SET case_title = COALESCE(EXCLUDED.case_title, public.cases.case_title),
        ...
        appeal_from_order = COALESCE(EXCLUDED.appeal_from_order, public.cases.appeal_from_order),
