-- Fix production RPC failures when pdf_queue URL columns are missing
-- Ensures columns exist, then replaces claim_pdf_batch to use them.
...
CREATE OR REPLACE FUNCTION public.claim_pdf_batch(
  p_thread_id uuid,
  p_batch_size int
)
