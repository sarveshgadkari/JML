-- 029_add_hearing_metrics_to_cases.sql
-- Add first/last hearing + duration + avg gap metrics to `public.cases`,
-- update admin import RPC to accept/populate them, and apply filing_date fallback.
...
ALTER TABLE public.cases
  ...
  ADD COLUMN IF NOT EXISTS avg_gap_between_hearings_days numeric;

-- Replace import RPC to accept new columns and apply filing_date fallback
...
BEGIN
  ...
  ON CONFLICT (case_number) DO UPDATE
    SET case_title = EXCLUDED.case_title,
        ...
        case_duration_days = EXCLUDED.case_duration_days,
        avg_gap_between_hearings_days = EXCLUDED.avg_gap_between_hearings_days,
        filing_date = COALESCE(EXCLUDED.filing_date, EXCLUDED.first_hearing_date, public.cases.filing_date),
