  ...
  FROM (
    ...
    mapped AS (
      ...
      JOIN public.lawyers l
        ON public.canonical_person_name(l.name) =
           public.canonical_person_name_with_initial_resolution(s.slot_lawyer_name, 'lawyer')
    ...
    ol_exp AS (
      SELECT
        lc.lawyer_id,
        NULLIF(btrim(o.nm_raw), '') AS nm_raw,
        public.canonical_person_name_with_initial_resolution(
          NULLIF(btrim(o.nm_raw), ''),
          'lawyer'
        ) AS nm_key
      FROM lawyer_case lc
      CROSS JOIN LATERAL unnest(lc.opp_lawyer_arr) AS o(nm_raw)
    ),
    ol_counts AS (
      SELECT
        lawyer_id,
        nm_key,
        COALESCE(MIN(l.name), MIN(ol_exp.nm_raw)) AS nm_display,
        COUNT(*)::integer AS cnt
      FROM ol_exp
      LEFT JOIN public.lawyers l
        ON public.canonical_person_name(l.name) = ol_exp.nm_key
      WHERE ol_exp.nm_raw IS NOT NULL
        AND ol_exp.nm_key IS NOT NULL
        AND btrim(ol_exp.nm_key) <> ''
      GROUP BY lawyer_id, nm_key
    ),
    ol_ranked AS (
      SELECT
        *,
        row_number() OVER (PARTITION BY lawyer_id ORDER BY cnt DESC, nm_display) AS rn
      FROM ol_counts
    ),
    ol_pivot AS (
      SELECT
        lawyer_id,
        MAX(nm_display) FILTER (WHERE rn = 1) AS chart_top_opp_lawyer_1_name,
        MAX(cnt) FILTER (WHERE rn = 1) AS chart_top_opp_lawyer_1_cases,
        MAX(nm_display) FILTER (WHERE rn = 2) AS chart_top_opp_lawyer_2_name,
        MAX(cnt) FILTER (WHERE rn = 2) AS chart_top_opp_lawyer_2_cases,
        MAX(nm_display) FILTER (WHERE rn = 3) AS chart_top_opp_lawyer_3_name,
        MAX(cnt) FILTER (WHERE rn = 3) AS chart_top_opp_lawyer_3_cases,
        MAX(nm_display) FILTER (WHERE rn = 4) AS chart_top_opp_lawyer_4_name,
        MAX(cnt) FILTER (WHERE rn = 4) AS chart_top_opp_lawyer_4_cases,
        MAX(nm_display) FILTER (WHERE rn = 5) AS chart_top_opp_lawyer_5_name,
    ...
    j_counts AS (
      SELECT
        lawyer_id,
        nm_key,
        COALESCE(MIN(j.name), MIN(j_exp.nm_raw)) AS nm_display,
        COUNT(*)::integer AS cnt
      FROM j_exp
      LEFT JOIN public.judges j
        ON public.canonical_person_name(j.name) = j_exp.nm_key
      WHERE j_exp.nm_raw IS NOT NULL
        AND j_exp.nm_key IS NOT NULL
        AND btrim(j_exp.nm_key) <> ''
      GROUP BY lawyer_id, nm_key
    ),
    j_ranked AS (
      SELECT
    ...
    j_pivot AS (
      SELECT
        lawyer_id,
        MAX(nm_display) FILTER (WHERE rn = 1) AS chart_top_judge_1_name,
        MAX(cnt) FILTER (WHERE rn = 1) AS chart_top_judge_1_cases,
        MAX(nm_display) FILTER (WHERE rn = 2) AS chart_top_judge_2_name,
        MAX(cnt) FILTER (WHERE rn = 2) AS chart_top_judge_2_cases,
        MAX(nm_display) FILTER (WHERE rn = 3) AS chart_top_judge_3_name,
        MAX(cnt) FILTER (WHERE rn = 3) AS chart_top_judge_3_cases,
        MAX(nm_display) FILTER (WHERE rn = 4) AS chart_top_judge_4_name,
        MAX(cnt) FILTER (WHERE rn = 4) AS chart_top_judge_4_cases,
        MAX(nm_display) FILTER (WHERE rn = 5) AS chart_top_judge_5_name,
    ...
    ol_set AS (
      SELECT
        ...
        COALESCE(MIN(l.name), MIN(s.nm_raw)) AS entity,
        COUNT(*)::integer AS tot,
        COUNT(*) FILTER (WHERE s.norm_outcome = 'settled')::integer AS st
      FROM (
        SELECT
          lc.lawyer_id,
          lc.norm_outcome,
          NULLIF(btrim(ol.nm), '') AS nm_raw,
          public.canonical_person_name_with_initial_resolution(ol.nm, 'lawyer') AS nm_key
        FROM lawyer_case lc
        CROSS JOIN LATERAL unnest(lc.opp_lawyer_arr) AS ol(nm)
      ) s
      LEFT JOIN public.lawyers l
        ON public.canonical_person_name(l.name) = s.nm_key
      WHERE s.nm_raw IS NOT NULL
        AND s.nm_key IS NOT NULL
        AND btrim(s.nm_key) <> ''
      GROUP BY s.lawyer_id, s.nm_key
      HAVING COUNT(*) >= 3
    ),
    j_set AS (
      SELECT
        s.lawyer_id,
        'judge'::text AS kind,
        COALESCE(MIN(j.name), MIN(s.nm_raw)) AS entity,
        COUNT(*)::integer AS tot,
        COUNT(*) FILTER (WHERE s.norm_outcome = 'settled')::integer AS st
      FROM (
        SELECT
          lc.lawyer_id,
          lc.norm_outcome,
          NULLIF(btrim(ju.nm), '') AS nm_raw,
          public.canonical_person_name_with_initial_resolution(ju.nm, 'judge') AS nm_key
        FROM lawyer_case lc
        CROSS JOIN LATERAL unnest(COALESCE(lc.judges_arr, ARRAY[]::text[])) AS ju(nm)
      ) s
      LEFT JOIN public.judges j
        ON public.canonical_person_name(j.name) = s.nm_key
      WHERE s.nm_raw IS NOT NULL
        AND s.nm_key IS NOT NULL
        AND btrim(s.nm_key) <> ''
      GROUP BY s.lawyer_id, s.nm_key
