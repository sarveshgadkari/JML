-- 040_table_analysis_light_refresh_lawyer_charts.sql
-- Same as 037 with post-step: refresh precomputed lawyer_analytics chart columns for touched lawyers (039).

BEGIN;

CREATE OR REPLACE FUNCTION public.admin_run_table_analysis_wide_light(p_case_numbers text[])
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF p_case_numbers IS NULL OR array_length(p_case_numbers, 1) IS NULL OR array_length(p_case_numbers, 1) = 0 THEN
    RETURN jsonb_build_object('status', 'ok', 'processed', 0);
  END IF;
  ...
  SELECT COALESCE(array_agg(DISTINCT bl.lawyer_id), ARRAY[]::uuid[]) INTO v_touched
  FROM (
    SELECT l.id AS lawyer_id
    FROM tmp_batch_cases t
    INNER JOIN public.cases c ON c.case_number = t.case_number
    CROSS JOIN LATERAL (
      SELECT unnest(
        CASE
          WHEN array_remove(ARRAY[
    ...
    INNER JOIN public.lawyers l ON public.canonical_person_name(l.name) = public.canonical_person_name(u.nm)
  ) bl;

  IF v_touched IS NOT NULL AND array_length(v_touched, 1) IS NOT NULL THEN
    PERFORM public.admin_refresh_lawyer_analytics_charts(v_touched);
  END IF;

  RETURN jsonb_build_object('status', 'ok', 'processed', v_processed);
END;
$$;

REVOKE ALL ON FUNCTION public.admin_run_table_analysis_wide_light(text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_run_table_analysis_wide_light(text[]) TO authenticated;
